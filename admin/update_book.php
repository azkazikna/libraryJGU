...
$books = query("SELECT *, books.description AS book_description, categories.description AS category_description FROM books INNER JOIN categories ON books.category = categories.id_category WHERE id_books = $id_books")[0];
//cek apakah tombol submit sudah ditekan?
if( isset($_POST["submit"])) {

    if( update($_POST) > 0 ) {
        echo "<script> alert('Update book success!'); document.location.href = 'books_data.php';</script>";
    } else {
        echo "<script> alert('Update book failed!'); document.location.href = 'books_data.php';</script>";
    }
...
<section class="dashboard">
        <div class="flex justify-center flex-col lg:flex-row gap-5 items-center">
            <div class="text w-full">
                ...
                <h1 class="text-darken text-5xl font-bold">Edit Book</h1>
            </div>
        </div>
        <hr class="my-8">
        <div class="w-full bg-white rounded-3xl p-4 lg:p-16 shadow-lg overflow-x-auto">
            <h1 class="text-xl lg:text-2xl font-bold">Edit data book</h1>
            <hr class="mb-3 mt-5">
            <form action="" method="post" enctype="multipart/form-data">
                <div class="form-group w-full">
                    ...
                    <input type="text" name="code_book" id="code_book" value="<?= $books["code_book"] ?>" class="bg-gray-300 dark:text-txtdark border bg-opacity-[40%] p-4 rounded-xl w-full" readonly>
                </div>
                <div class="flex flex-col lg:flex-row gap-2 mt-5">
                    <div class="form-group w-full">
                        ...
                        <input type="hidden" name="id_user" value="<?= $books["id_user"] ?>">
                        <input type="hidden" name="id_books" value="<?= $books["id_books"] ?>">
                        <input type="hidden" name="oldCover" value="<?= $books["cover"] ?>">
                        <input type="text" name="author" id="author" class="bg-white dark:text-txtdark border bg-opacity-[40%] p-4 rounded-xl w-full" placeholder="Enter author name here" value="<?= $books["author"] ?>">
                    </div>
                    <div class="form-group w-full">
                        <label for="title" class="text-dark dark:text-txtdark">Title <span class="text-red-500">*</span></label><br>
                        <input type="text" name="title" id="title" class="bg-white dark:text-txtdark border bg-opacity-[40%] p-4 rounded-xl w-full" placeholder="Enter title name here" value="<?= $books["title"] ?>">
                    </div>
                </div>
                    <div class="form-group w-full">
                        ...
                        <select type="text" name="category" id="category" class="bg-white dark:text-txtdark border bg-opacity-[40%] p-4 rounded-xl w-full" placeholder="Enter title name here">
                            <option value="<?= $books["id_category"]?>"><?= $books["name"]?></option>
                ...
                <div class="form-group w-full mt-2 lg:mt-5">
                    ...
                    <textarea type="text" name="description" id="description" class="bg-white dark:text-txtdark border bg-opacity-[40%] p-4 rounded-xl w-full" placeholder="Add description..."><?= $books["book_description"] ?></textarea>
                </div>
                <div class="form-group w-full">
                    <label for="cover" class="text-dark dark:text-txtdark">Cover <span class="text-red-500">:</span></label><br>
                    <img src="../dist/img/cover/<?= $books["cover"] ?>" alt="" class="mb-2 w-24 h-36 block object-cover">
